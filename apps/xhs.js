document.addEventListener('DOMContentLoaded', () => {
    ...
    // 当前激活的首页Tab（discover/follow/local）
    let currentHomeTab = 'discover';

    // 生成状态隔离：按tab/笔记ID跟踪
    let isDiscoverGenerating = false;
    let isFollowGenerating = false;
    let commentGeneratingNoteId = null;
    ...
    // 获取所有可用角色（含NPC），返回统一格式
    function getAllXhsCharacters() {
    ...
    // 通过ID查找角色（含NPC），返回统一格式或null
    function findXhsCharById(charId) {
        const chatsMap = window.state.chats || {};
        if (chatsMap[charId] && !chatsMap[charId].isGroup) {
            const chat = chatsMap[charId];
            return {
                id: chat.id, name: chat.name,
                avatar: chat.settings?.aiAvatar || chat.avatar || '',
                persona: chat.settings?.aiPersona || '',
                isNpc: false, ownerChatId: null, chatData: chat
            };
        }
        for (const chat of Object.values(chatsMap)) {
            if (chat.isGroup) continue;
            const npc = (chat.npcLibrary || []).find(n => n.id === charId);
            if (npc) {
                return {
                    id: npc.id, name: npc.name,
                    avatar: npc.avatar || '',
                    persona: npc.persona || '',
                    isNpc: true, ownerChatId: chat.id, chatData: chat
                };
            }
        }
        return null;
    ...
    window.renderXhsProfileContent = async function (tabName) {
        ...
        if (tabName === 'notes') {
            ...
            // 使用瀑布流渲染，为自己的笔记绑定长按删除
            renderWaterfall(container, myNotes, (note) => {
                const card = createXhsCard(note);
                card.onclick = null;
                bindLongPress(card, () => {
                    showXhsConfirmGeneral(
                        `确定要删除笔记「${note.title}」吗？<br>删除后不可恢复。`,
                        '删除',
                        async () => {
                            await deleteXhsNote(note.id);
                            const toast = document.createElement('div');
                            toast.textContent = '笔记已删除';
    ...
    async function buildXhsGenerationPrompt(character) {
        ...
        const _globalWbXhs1 = window.WorldBookModule.getGlobalWorldBooksContext(linkedBookIds || []);
        if (_globalWbXhs1) worldBookContext += _globalWbXhs1;
        ...
        2. 【重要-视角限制-严格遵守-最高优先级规则】
           - 路人笔记和路人评论必须以路人自己的独立视角撰写。路人是完全独立的第三方小红书用户，拥有各自不同的身份、经历和生活背景。
           - 绝对不能以“${myName}”（手机主人/用户）的口吻、视角或身份来写路人内容。
           - 路人的authorName绝对不能使用“${myName}”或与其相似的名字。
           - 路人绝对不能以聊天记录中出现的任何角色的名字、身份、视角、口吻来发布笔记或评论。这包括但不限于：不能使用角色的名字作为authorName或评论者名字，不能模仿角色的说话方式，不能以角色的身份讲述经历。聊天记录中提到的所有人物名字对路人来说都是不存在的陌生人。
        3. 内容风格必须极度符合小红书特点：
        ...
        4. "imagePrompt": 为每条笔记生成一个简短的、描述性的**英文**图片提示词，用于AI生图（例如 "delicious matcha latte art, cozy cafe, realistic"）请注意提示词当中【绝对不允许】出现【人物】。
        5. "stats": 随机生成合理的点赞数（likes）和收藏数（collects）。
        6. "comments": 为每条笔记生成 2-3 条精彩的模拟评论（包含评论者名字和内容）。评论同样遵守视角限制：路人评论者是完全独立的第三方小红书用户，绝对不能使用"${myName}"的身份或口吻，绝对不能以聊天记录中出现的任何角色的名字、身份或视角来发评论。评论者的名字必须是随机编造的网名，不能与任何角色名或"${myName}"相同或相似。评论内容不能涉及聊天记录中的事件。
        7. "publishTime": 每条笔记的发布时间，格式为 "YYYY-MM-DD HH:mm"。当前时间为 ${new Date().toLocaleString('zh-CN')}。请根据笔记内容合理设定发布时间（在过去24小时内）。如果笔记内容与角色聊天记录有关联，发布时间必须在聊天事件之后，符合时间逻辑。
    ...
    async function generateXhsNotes(isAuto = false) {
        ...
        isDiscoverGenerating = true;
        if (refreshBtn && currentHomeTab === 'discover') refreshBtn.classList.add('spinning');

        try {
            const apiConfig = window.getApiConfigForFunction('xhs');
            const { proxyUrl, apiKey, model } = apiConfig;
            if (!proxyUrl || !apiKey || !model) {
                if (!isAuto) alert("请先配置 API 设置！");
                isDiscoverGenerating = false;
                if (refreshBtn && currentHomeTab === 'discover') refreshBtn.classList.remove('spinning');
                return false;
            }

            let selectedChar = null;
            const allowedPosters = window.state.xhsSettings?.allowedPosters || [];
            const availableChars = getAllXhsCharacters();
            ...
            if (isGemini) {
                ...
                const body = {
                    ...
                    generationConfig: { ...window.buildModelParams(apiConfig) }
                };
            ...
            if (result && result.notes && Array.isArray(result.notes)) {
                ...
                await Promise.all(result.notes.map(async (note) => {
                    ...
                    if (note.isCharacter && selectedChar) {
                        note.authorAvatar = selectedChar.avatar || selectedChar.settings?.aiAvatar || "https://i.postimg.cc/qRqpK5kP/anime-avatar.jpg";
                    ...
                    if (note.comments && Array.isArray(note.comments)) {
                        const charMap = {};
                        getAllXhsCharacters().forEach(ch => {
                            charMap[ch.name] = ch.avatar;
                        });
            ...
            isDiscoverGenerating = false;
            if (refreshBtn && currentHomeTab === 'discover') {
    ...
    async function generateXhsSearchNotes(query) {
        try {
            ...
            3. 【重要-视角限制-严格遵守-最高优先级规则】
           - 所有笔记和评论均由独立的第三方小红书用户撰写，每个作者拥有各自不同的身份、经历和视角。
           - 绝对不能以"${myName}"（即手机主人）的视角或口吻来撰写任何内容。
           - 作者名字和评论者名字绝对不能使用"${myName}"或与其相似、谐音、缩写的名字，也不能使用任何聊天记录中出现过的角色名字、身份或视角。
           - 【禁止冒充角色】不能以聊天中出现的任何角色的身份、视角或口吻来发布笔记或评论。聊天记录中的所有人物对这些作者来说完全不存在。
           - 评论者同样必须是完全独立的路人，不能使用任何角色名字或"${myName}"作为评论者名字。
            4. 作者名字要随机多样，像真实的网友昵称（不要叫“小红书助手”之类的）。
            5. "isCharacter" 设为 false。
            6. "imagePrompt": 为每条笔记生成一个简短的、描述性的英文图片提示词，用于AI生图。即使关键词是中文，提示词也必须翻译成英文。提示词中【绝对不允许】出现【人物】（person, girl, man, people等），重点描述物体、场景、氛围、光线、构图。
            7. "stats": 随机生成合理的点赞数和收藏数。
            8. "comments": 每条笔记生成 2-3 条精彩评论，评论要真实互动，有短有长，也可以带emoji。
    ...
    async function generateFollowedUsersNotes() {
        isFollowGenerating = true;
        if (refreshBtn && currentHomeTab === 'follow') refreshBtn.classList.add('spinning');
        ...
        try {
            ...
            if (!proxyUrl || !apiKey || !model) {
                alert("请先配置 API 设置！");
                isFollowGenerating = false;
                if (refreshBtn && currentHomeTab === 'follow') refreshBtn.classList.remove('spinning');
                return false;
            }
            ...
            // 获取角色信息用于生成
            const allChars = getAllXhsCharacters();
            const chatsMap = window.state.chats || {};
            const charactersInfo = selectedProfiles.map(([name, profile]) => {
                const charMatch = allChars.find(c => c.name === name);
                return {
                    name: name,
                    avatar: profile.avatar,
                    bio: profile.bio || '',
                    persona: charMatch?.persona || '',
                    chatData: charMatch?.chatData || null
                };
            });
...
`;
            ...
            const _globalWbXhs2 = window.WorldBookModule.getGlobalWorldBooksContext(linkedBookIds);
            if (_globalWbXhs2) worldBookContext += _globalWbXhs2;
...
`;
            ...
            isFollowGenerating = false;
            if (refreshBtn && currentHomeTab === 'follow') {
                refreshBtn.classList.remove('spinning');
            }
        }
    }
    ...
    async function handleXhsRegenImage(note) {
        const promptFunc = window.showCustomPrompt;
        if (!promptFunc) return;

        const userPrompt = await promptFunc(
            '重新生成图片',
            '请输入图片描述提示词（将自动翻译为英语后调用 NovelAI 生图）',
            note.imagePrompt || '',
            'textarea'
        );
        ...
        const translatedPrompt = window.translatePromptToEnglish
            ? await window.translatePromptToEnglish(userPrompt.trim())
            : userPrompt.trim();
        ...
        try {
            ...
            if (model.includes('nai-diffusion-4')) {
                requestBody = {
                    input: translatedPrompt, model, action: 'generate',
                    parameters: {
                        ...commonParams, params_version: 3,
                        negative_prompt: negativePrompt,
                        v4_prompt: { caption: { base_caption: translatedPrompt, char_captions: [] }, use_coords: false, use_order: true },
            ...
            note.imageUrl = imageDataUrl;
            if (note.images && note.images.length > 0) note.images[0] = imageDataUrl;
            if (window.db && window.db.xhsNotes && !note.isSearchTemp) {
                await window.db.xhsNotes.put(note);
            }

            const mediaWrap = document.getElementById('xhs-detail-images');
            const imgEl = mediaWrap ? mediaWrap.querySelector('.xhs-media-img') : null;
            if (imgEl) imgEl.src = imageDataUrl;

            if (window.showCustomAlert) window.showCustomAlert('生成成功', '图片已更新！');
        } catch (e) {
            console.error('XHS 重新生图失败:', e);
            if (window.showCustomAlert) window.showCustomAlert('生成失败', `NovelAI 图片生成出错：${e.message}`);
        }
    }

    async function openXhsNoteDetail(noteInput) {
        if (!views.noteDetail) return;

        // 从数据库获取最新数据，避免使用卡片闭包中的过期对象（如后台生成的评论）
        let note = noteInput;
        if (window.db && window.db.xhsNotes && !noteInput.isSearchTemp) {
            const freshNote = await window.db.xhsNotes.get(noteInput.id);
            if (freshNote) note = freshNote;
        }
        ...
        mediaWrap.innerHTML = `<div style="position:relative;">
            <img src="${detailCoverImage}" class="xhs-media-img">
            <div class="xhs-regen-img-btn" title="重新生成图片">
        ...
        const regenBtn = mediaWrap.querySelector('.xhs-regen-img-btn');
        if (regenBtn) {
            regenBtn.onclick = async (e) => {
                e.stopPropagation();
                await handleXhsRegenImage(note);
            };
        }
        ...
        async function generateCommentsForNote() {
            const thisNoteId = note.id;
            commentGeneratingNoteId = thisNoteId;
            ...
            try {
                ...
                if (s?.enableChatMemory !== false) {
                    ...
                    // 获取角色的聊天记录（包括链接的记忆互通聊天），NPC使用所属角色的聊天
                    const getCharacterMemory = (char) => {
                        const chatId = char.ownerChatId || char.id;
                        const chat = window.state.chats[chatId];
                        if (!chat) return [];

                        const memories = [];

                        // 1. 获取角色自己的聊天记录（NPC用所属角色的chatId去重）
                        if (chat.history && chat.history.length > 0 && !processedChatIds.has(chatId)) {
                ...
                if (allCommentingChars.length > 0) {
                    const settingsParts = allCommentingChars.map(char => {
                        const persona = char.persona || char.settings?.aiPersona || "普通用户";
                ...
                const _globalWbXhs3 = window.WorldBookModule.getGlobalWorldBooksContext(linkedBookIds);
                if (_globalWbXhs3) worldBookContext += _globalWbXhs3;
                ...
                10. 评论风格要求:
                   ...
                   - 有人设的角色要符合其人设特点
                   - 路人评论要多样化，有正面也可以有中性评价。路人是完全独立的第三方用户，绝对不能以"${myName}"（手机主人）的视角或口吻发评论。【禁止冒充角色】路人绝对不能以聊天记录中出现过的任何角色的名字、身份、视角或口吻发评论。路人的网名不能与任何角色名或"${myName}"相同、相似、谐音或缩写。路人评论内容不能涉及聊天记录中的事件或人物关系。
                   - 评论长度适中，有长有短
                ...
                if (result && result.comments && Array.isArray(result.comments)) {
                    ...
                    for (const genComment of result.comments) {
                        ...
                        if (genComment.isCharacter) {
                            ...
                            avatar = char?.avatar || `https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(genComment.user)}`;
                        } else {
                ...
                if (commentGeneratingNoteId === thisNoteId) {
                    commentGeneratingNoteId = null;
                }
                const currentBtn = document.getElementById('xhs-generate-comments-btn');
                if (currentBtn && views.noteDetail.dataset.currentNoteId == thisNoteId) {
                    currentBtn.classList.remove('loading');
                    currentBtn.querySelector('span').textContent = '生成评论';
                }
            }
        }

        // 绑定生成评论按钮（重置状态，避免上一个笔记的生成状态泄漏）
        ...
        if (generateCommentsBtn) {
            ...
            newGenerateBtn.classList.remove('loading');
            const spanEl = newGenerateBtn.querySelector('span');
            if (spanEl) spanEl.textContent = '生成评论';
    ...
    function showXhsDeleteDiscoverConfirm() {
                ...
                <div class="xhs-confirm-message">
                    ...
                    <div style="margin-top: 15px; text-align: left; font-size: 14px;">
                        <label style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px; cursor: pointer;">
                            <input type="checkbox" id="xhs-del-mine" style="width: 16px; height: 16px;">
                            <span>同时清除自己发布的笔记</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px; cursor: pointer;">
        ...
        modal.querySelector('.xhs-confirm-ok').onclick = async () => {
            const deleteMine = document.getElementById('xhs-del-mine').checked;
            ...
            if (window.db && window.db.xhsNotes) {
                try {
                    ...
                    for (const note of discoverNotes) {
                        ...
                        const isMine = note.isMine === true;

                        // 未勾选 "同时清除自己发布的" → 保留自己的笔记
                        if (!deleteMine && isMine) continue;
    ...
    topTabItems.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            currentHomeTab = target;
            // 切换tab时同步刷新按钮旋转状态
            if (refreshBtn) {
                const shouldSpin = (target === 'discover' && isDiscoverGenerating) ||
                                   (target === 'follow' && isFollowGenerating);
                refreshBtn.classList.toggle('spinning', shouldSpin);
            }
            if (feeds.discover) feeds.discover.style.display = 'none';
            if (feeds.follow) feeds.follow.style.display = 'none';
            // 隐藏附近页占位
    ...
    function initXhsNoteCreator() {
        ...
        function renderMentionList(filter = '') {
            ...
            const characters = getAllXhsCharacters();
            ...
            filtered.forEach(char => {
                ...
                const avatar = char.avatar || "https://api.dicebear.com/7.x/notionists/svg?seed=" + encodeURIComponent(char.name);
                item.innerHTML = `
                    <img src="${avatar}" alt="${char.name}">
                    <div class="xhs-mention-item-info">
                        <div class="xhs-mention-item-name">${char.name}</div>
                        <div class="xhs-mention-item-desc">${char.persona?.substring(0, 30) || '角色'}...</div>
                    </div>
                `;
                item.onclick = () => {
        ...
        if (tagInput) {
            ...
            // 将input包裹在form中，利用表单submit事件可靠捕获移动端软键盘回车
            const tagForm = document.createElement('form');
            tagForm.style.display = 'contents';
            tagInput.parentNode.insertBefore(tagForm, tagInput);
            ...
            // 桌面端回车（form submit已覆盖，此处兜底处理IME组合状态）
            let isComposing = false;
            tagInput.addEventListener('compositionstart', () => { isComposing = true; });
            tagInput.addEventListener('compositionend', () => { isComposing = false; });

            tagInput.addEventListener('keydown', (e) => {
                if (!isComposing && (e.key === 'Enter' || e.keyCode === 13)) {
                    e.preventDefault();
                    if (tagInput.value.trim()) {
                        addTag(tagInput.value.trim());
                        tagInput.value = '';
    ...
    // 智能匹配角色（含NPC） - 根据名称在角色列表中查找匹配的角色
    function findMatchingCharacter(userName) {
        if (!userName) return null;
        const allChars = getAllXhsCharacters();

        const matched = allChars.find(c => c.name === userName);
        if (matched) {
            return {
                name: matched.name,
                avatar: matched.avatar,
                isCharacter: true,
                chatData: matched.chatData,
    ...
    async function generateCharacterProfile(authorName, authorAvatar) {
        ...
        const matchedChar = findMatchingCharacter(authorName);
    ...
    async function generateCharacterCollections(profile) {
        ...
        // 获取角色人设（含NPC）
        let characterContext = "";
        const matchedCharForColl = findMatchingCharacter(authorName);
        if (matchedCharForColl && matchedCharForColl.persona) {
            characterContext = `【角色人物设定】：\n${matchedCharForColl.persona}`;
        }
        ...
        let chatContext = "";
        const matchedChatForColl = matchedCharForColl ? window.state.chats[matchedCharForColl.ownerChatId || matchedCharForColl.id] : null;
        if (s?.enableChatMemory !== false && matchedChatForColl && matchedChatForColl.history && matchedChatForColl.history.length > 0) {
            const myName = s?.nickname || "MOMO";
            const limit = matchedChatForColl.settings?.maxMemory || 10;
            const recentMsgs = matchedChatForColl.history.slice(-limit);
        ...
        const _globalWbXhs4 = window.WorldBookModule.getGlobalWorldBooksContext(linkedBookIds);
        if (_globalWbXhs4) worldBookContext += _globalWbXhs4;
...
7. 【重要-视角限制-严格遵守-最高优先级规则】收藏夹中的笔记和评论均由独立的第三方小红书用户撰写。不能以"${myName}"（手机主人）的视角或口吻来写任何内容。【禁止冒充角色】绝对不能以聊天记录中出现过的任何角色的名字、身份、视角或口吻来发布笔记或评论。聊天记录中提到的所有人物名字对这些作者来说完全不存在。每个作者和评论者都是拥有独立身份和经历的真实网友，与聊天记录完全无关。
8. "imagePrompt": 为每条笔记生成一个详细的英文图片提示词，用于AI生图。提示词中【绝对不允许】出现【人物】（person, girl, man, people等），重点描述物体、场景、氛围、光线、构图。
9. "stats": 随机生成合理的点赞数（likes）和收藏数（collects）。
10. "comments": 为每条笔记生成 2-3 条精彩的模拟评论（包含评论者名字和内容），评论要真实互动，有短有长，可以带emoji。评论遵守视角限制：路人评论者绝对不能使用"${myName}"的身份或口吻，也绝对不能以聊天记录中任何角色的名字、身份或视角来发评论。评论者名字必须是随机编造的网名。
11. "location": 笔记的发布地点，格式为"城市, 地点"。
...
${isUpdate ? '说明：isExisting为true表示这是已有的收藏夹（使用原名称，只生成要新增的笔记），isExisting为false表示全新的收藏夹。\n' : ''}请只返回JSON数据，不要包含markdown代码块标记。`;
    ...
    async function renderUserProfile(profile) {
        ...
        // 填充主页背景
        const headerEl = views.userProfile.querySelector('.xhs-user-profile-header');
        if (headerEl) {
            if (profile.backgroundUrl) {
                headerEl.style.backgroundImage = `linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,0.5) 100%), url(${profile.backgroundUrl})`;
    ...
    function showUserProfileSettingsModal(profile) {
        ...
        modal.innerHTML = `
            <div class="xhs-user-profile-settings-content">
                ...
                <div class="xhs-user-profile-settings-body">
                    ...
                    <div class="xhs-user-profile-settings-item">
                        <label>主页背景 (图片URL)</label>
                        <input type="text" id="xhs-user-settings-bg" value="${profile.backgroundUrl || ''}" placeholder="输入背景图片URL">
                        ${profile.backgroundUrl ? `<img src="${profile.backgroundUrl}" style="width:100%;max-height:80px;object-fit:cover;border-radius:8px;margin-top:6px;">` : ''}
                    </div>
                </div>
        ...
        modal.querySelector('.xhs-user-profile-settings-save').onclick = async () => {
            ...
            const updatedProfile = {
                ...
                backgroundUrl: modal.querySelector('#xhs-user-settings-bg').value.trim()
            };

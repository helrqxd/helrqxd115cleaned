document.addEventListener('DOMContentLoaded', () => {
    ...
    async function computeAndApplyFansFluctuation() {
        ...
        const now = Date.now();
        const currentHour = floorToHour(now); // 当前整点

        // 如果当前时间还没到下一个整点，不执行
        // （即：上次计算的整点 >= 当前整点 → 本整点已算过）
        const lastTime = s.fansFluctuationData.lastFluctuationTime || 0;
        const lastHour = lastTime > 0 ? floorToHour(lastTime) : 0;

        if (lastHour >= currentHour) {
            return; // 当前整点已计算过
        }

        console.log('[XHS] 开始整点粉丝浮动计算...');
        ...
        if (lastTime === 0) {
            ...
            // 从上次计算的下一个整点开始
            startHour = lastHour + ONE_HOUR;
        }

        // 限制最多补算24小时（防止长期离线后一次性计算过多）
        const maxBackfillHours = 24;
        if (currentHour - startHour > maxBackfillHours * ONE_HOUR) {
            startHour = currentHour - maxBackfillHours * ONE_HOUR;
            console.log(`[XHS] 超过${maxBackfillHours}小时未计算，只补算最近${maxBackfillHours}小时`);
        }

        // 逐个整点执行计算
        let hourCount = 0;
        for (let hourTs = startHour; hourTs <= currentHour; hourTs += ONE_HOUR) {
            await computeFansForOneHour(s, hourTs);
            hourCount++;
        }
        ...
        // 记录本次计算完成时间为当前整点
        s.fansFluctuationData.lastFluctuationTime = currentHour;
        await saveXhsSettings({});
        updateMessageBadges();

        console.log(`[XHS] 粉丝浮动完成，共计算 ${hourCount} 个整点`);
    }
    ...
    function startFansFluctuationTimer() {
        ...
        console.log('[XHS] 启动粉丝浮动定时器（整点触发模式）');
        // 每30秒检查一次是否到达新的整点

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function buildXhsGenerationPrompt(character) {
        ...
        2. 【重要-视角限制】路人笔记和路人评论必须以路人自己的独立视角撰写。路人是完全独立的第三方小红书用户，拥有各自不同的身份、经历和生活背景。绝对不能以聊天记录中"${myName}"的口吻、视角或身份来写路人内容。聊天记录仅供角色（isCharacter=true）的笔记参考，路人对这些聊天内容一无所知。
        3. 内容风格必须极度符合小红书特点：
        ...
        6. "comments": 为每条笔记生成 2-3 条精彩的模拟评论（包含评论者名字和内容）。评论同样遵守视角限制：路人评论者是独立的小红书用户，不能使用"${myName}"的身份或口吻。
    ...
    async function generateXhsSearchNotes(query) {
        try {
            ...
            3. 【重要-视角限制】所有笔记和评论均由独立的第三方小红书用户撰写，每个作者拥有各自不同的身份、经历和视角。绝对不能以"${myName}"（即手机主人）的视角或口吻来撰写任何内容。
        ...
        async function generateCommentsForNote() {
            ...
            try {
                ...
                10. 评论风格要求:
                   ...
                   - 有人设的角色要符合其人设特点
                   - 路人评论要多样化，有正面也可以有中性评价。路人是完全独立的第三方用户，绝对不能以"${myName}"（手机主人）的视角或口吻发评论
                   - 评论长度适中，有长有短
...
7. 【重要-视角限制】收藏夹中的笔记和评论均由独立的第三方小红书用户撰写，不能以"${myName}"（手机主人）的视角或口吻来写任何内容。每个作者和评论者都是拥有独立身份和经历的真实网友。

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateXhsNotes(isAuto = false) {
        ...
        try {
            const { proxyUrl, apiKey, model, temperature } = window.getApiConfigForFunction('xhs');
    ...
    async function generateXhsSearchNotes(query) {
        try {
            const { proxyUrl, apiKey, model, temperature } = window.getApiConfigForFunction('xhs'); // 获取 temperature
            if (!proxyUrl || !apiKey || !model) {

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function buildXhsGenerationPrompt(character) {
        ...
        2. 【重要-视角限制-严格遵守】
           ...
           - 路人绝对不能以聊天记录中出现的任何角色的名字、身份或视角来发布笔记或评论。聊天记录仅供isCharacter=true的角色笔记参考，路人对这些角色和聊天内容完全不知情。
           - 路人不能提及、引用或暗示聊天记录中的任何私密对话内容。
        3. 内容风格必须极度符合小红书特点：
        ...
        6. "comments": 为每条笔记生成 2-3 条精彩的模拟评论（包含评论者名字和内容）。评论同样遵守视角限制：路人评论者是独立的小红书用户，不能使用"${myName}"的身份或口吻，也不能以聊天记录中出现的任何角色的名字或身份来发评论。
    ...
    async function generateXhsSearchNotes(query) {
        try {
           ...
           - 作者名字绝对不能使用"${myName}"或与其相似的名字，也不能使用任何聊天记录中出现过的角色名字。
           - 不能以聊天中出现的任何角色的身份或视角来发布内容。
        ...
        async function generateCommentsForNote() {
            ...
            try {
                ...
                10. 评论风格要求:
                   ...
                   - 有人设的角色要符合其人设特点
                   - 路人评论要多样化，有正面也可以有中性评价。路人是完全独立的第三方用户，绝对不能以"${myName}"（手机主人）的视角或口吻发评论，也不能以聊天记录中出现过的角色的名字或身份发评论。路人的网名不能与任何角色名或"${myName}"相同或相似
                   - 评论长度适中，有长有短
...
7. 【重要-视角限制-严格遵守】收藏夹中的笔记和评论均由独立的第三方小红书用户撰写。不能以"${myName}"（手机主人）的视角或口吻来写任何内容。也不能以聊天记录中出现过的任何角色的名字、身份或视角来发布笔记或评论。每个作者和评论者都是拥有独立身份和经历的真实网友。
...
10. "comments": 为每条笔记生成 2-3 条精彩的模拟评论（包含评论者名字和内容），评论要真实互动，有短有长，可以带emoji。评论遵守视角限制：路人评论者不能使用"${myName}"的身份或口吻。

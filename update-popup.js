        ...
        sections: [
            {
                title: '🐛 Bug 修复',
                items: [
                    '修复QQ聊天API错误处理，重复读取响应体报错',
                    '修复小红书角色主页收藏夹生成时部分聊天变量未正确匹配，可能导致生成失败',
                ]
            },
            {
                title: '✨ 优化',
                items: [
                    '优化小红书笔记、评论、搜索和角色收藏夹生成prompt，严格禁止路人使用聊天记录中的人物、用户或角色视角（这个真的调上下文调到力竭惹，如果还有偶发现象麻烦大家世界书肘一肘，还是很频繁的话可以带截图私信戳我）',
                    '优化小红书角色主页收藏夹生图逻辑，与主页生成笔记保持一致；没有Pollinations API key时直接使用占位图，不再发起生图调用',
                    '优化小剧场消息显示样式，支持区分行动文本和对话文本，并分别添加字体与颜色设置',
                    '优化小剧场存档数据存储，防止空间不足导致数据丢失',
                    '优化约会大作战完成弹窗，关闭弹窗后仍可查看最后一次角色回复、重新生成回复，并可手动退出',
                ]
            },
            {
                title: '🚀 新功能',
                items: [
                    '新增游戏大厅世界书绑定，支持绑定到所有游戏或指定游戏',
                    '新增小剧场剧本数据同步，修改剧本后，会自动同步更新相关存档',
                    '新增小剧场文字颜色与字体设置',
                    '新增约会大作战历史记录，可查看本次约会的前文记录',
                ]
            }
        ]
    },
